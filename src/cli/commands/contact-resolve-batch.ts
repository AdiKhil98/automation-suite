...
const SKIP_LABEL: Record<SkipReason, string> = {
  ...
  chain_exhausted: 'contact-enrichment chain conclusively exhausted',
};
...
export async function contactResolveBatchCommand(ctx: CliContext, opts: ContactResolveBatchOptions, deps: ContactResolveBatchDeps = {}): Promise<void> {
  ...
  // ---- LIVE ----
  if (services.size === 0) {
    throw new AppError('NO_PROVIDERS_AVAILABLE', 'contact-resolve-batch: no cascade provider is configured (need CONTACT_ENRICHMENT_ENABLED + at least one of INSTANTLY_API_KEY/HUNTER_API_KEY).');
  }
  ...
  runLoop: for (const target of selected) {
    ...
    for (const providerName of target.nextSteps) {
      ...
      try {
        ...
        if (result.outcome === 'VERIFIED' && result.accepted) {
          verified = { fullName: result.accepted.fullName, email: result.accepted.email };
          break;
        }
      } catch (err) {
        errored = true;
  ...
  console.log(
    `\n  done: ${String(verifiedCount)} verified, ${String(unresolvedCount)} unresolved, ${String(erroredCount)} errored. requests=${String(totalRequests)}/${String(maxTotalRequests)} credits=${String(totalCredits)}/${String(maxTotalCredits)}.`,
  );
