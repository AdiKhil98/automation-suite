...
const BASE_TRANSITIONS: Record<LeadStatus, LeadStatus[]> = {
  ...
  QUALIFIED: ['READY_FOR_CAPTURE'],
  READY_FOR_ENRICHMENT: ['ENRICHED', 'NEEDS_MANUAL_REVIEW', 'FAILED'],
  ENRICHED: ['READY_FOR_QUALIFICATION'],
  READY_FOR_CAPTURE: ['CAPTURED', 'NEEDS_MANUAL_REVIEW', 'FAILED'],
  ...
  // Wording approval keeps it here; Phase 11 deploy+verify+finalize advances to FINALIZED_EMAIL_PENDING;
  // rejection or an invalid artifact/verification routes to REJECTED / NEEDS_MANUAL_REVIEW.
