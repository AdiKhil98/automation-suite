...
describe('classifyLead', () => {
  it('skips a non-QUALIFIED lead', () => {
    const d = classifyLead(lead({ status: 'NEW' }), DOMAIN, CANDIDATES, [], BOTH, null);
    expect(d).toEqual({ eligible: false, reason: 'not_qualified' });
  });

  it('skips a lead with no verified official_domain', () => {
    const d = classifyLead(lead(), null, CANDIDATES, [], BOTH, null);
    expect(d).toEqual({ eligible: false, reason: 'no_verified_domain' });
  });

  it('skips a lead with no known candidates (absent or empty)', () => {
    expect(classifyLead(lead(), DOMAIN, undefined, [], BOTH, null)).toEqual({ eligible: false, reason: 'no_known_candidates' });
    expect(classifyLead(lead(), DOMAIN, [], [], BOTH, null)).toEqual({ eligible: false, reason: 'no_known_candidates' });
  });
  ...
  it('skips a lead that already holds a VERIFIED contact from any past provider/candidate set', () => {
    ...
    const d = classifyLead(lead(), DOMAIN, CANDIDATES, results, BOTH, null);
  ...
  it('is eligible with both providers as next steps when nothing has been attempted', () => {
    const d = classifyLead(lead(), DOMAIN, CANDIDATES, [], BOTH, 62);
  ...
  it('a resolved step for a provider NOT available this run is simply not offered, but does not itself exhaust the lead', () => {
    ...
    const d = classifyLead(lead(), DOMAIN, CANDIDATES, results, ['hunter'], null);
    expect(d.eligible).toBe(true);
...
describe('selectResolveBatchTargets', () => {
  it('orders eligible leads by strongest opportunity score first, then createdAt/id tie-break, and caps at limit', () => {
    ...
    const { selected, skipped } = selectResolveBatchTargets(
      leads,
      { officialDomainByLead, candidatesByLead, existingResultsByLead, maxOpportunityScoreByLead, availableProviders: BOTH },
      { limit: 2 },
    );
  ...
  it('reports a full skip breakdown by reason', () => {
    const leads = [lead({ id: 'a', status: 'NEW' }), lead({ id: 'b' })]; // b: QUALIFIED but no domain
    const officialDomainByLead = new Map<string, string | null>([['b', null]]);
    const { selected, skipped } = selectResolveBatchTargets(
      leads,
      { officialDomainByLead, candidatesByLead: new Map(), existingResultsByLead: new Map(), maxOpportunityScoreByLead: new Map(), availableProviders: BOTH },
      { limit: 10 },
    );
